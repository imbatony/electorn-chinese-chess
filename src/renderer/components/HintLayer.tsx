...
export const HintLayer = React.memo(({ select, board, red, lastMove, rotation, selected, availableMovement }: HintLayerProps) => {
    const showSelect = select.x >= 0 && select.y >= 0 && board[select.y][select.x] > 0 && (PieceArray[board[select.y][select.x] - 1].IsRed() === red)
