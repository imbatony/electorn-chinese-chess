...
import { GetELEEYEEngine,UCCI } from "./UCCI";
    ...
    submenu: [
      {
        ...
        label: "悔棋",
        click: () => {
          mainWindow.webContents.send(OP_BACK);
        },
        enabled: false,
      },
      {
        ...
        label: "重新开始",
        click: () => {
          mainWindow.webContents.send(OP_RESTART);
        },
        enabled: false,
      },
      {
        ...
        label: "翻转",
        click: () => {
          mainWindow.webContents.send(OP_ROTATION);
        },
        enabled: false,
      },
      {
        ...
        label: "打开/关闭音乐",
        click: () => {
          mainWindow.webContents.send(OP_TOGGLE_BGM);
        },
        enabled: true,
      },
      {
...
ipcMain.handle(QueryMoveKey, async (event, fenStr, difficulty) => {
...
ipcMain.on(BoardStatusKey, (_evt, status: BoardStatus) => {
  boardStatus = status;
  console.log(status);
  menu.getMenuItemById(OP_BACK).enabled = status.canBack;
  menu.getMenuItemById(OP_RESTART).enabled = true;
  menu.getMenuItemById(OP_ROTATION).enabled = true;
  menu.getMenuItemById(OP_COYPY_FEN).enabled = true;
});
