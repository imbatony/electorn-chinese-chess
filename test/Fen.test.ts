...
test('legacy isChecking keeps flying-general detection separate', () => {
  const fen = new FEN('4k4/9/9/9/9/9/9/9/9/4K4 w');
  expect(fen.isChecking(true)).toBe(false);
  expect(fen.isChecking(false)).toBe(false);
  expect(fen.isKingFacing()).toBe(true);
});
